 ...
 * @returns {Array<T>}
 */
export function plainsToInstances(Type, plains) {
  if (!(plains instanceof Array)) {
    throw TypeError("For non-arrays, `plainToInstance` should be used");
  }
  const array = [];
...
export function instancesToPlains(instances) {
  if (!(instances instanceof Array)) {
    throw TypeError("For non-arrays, `instanceToPlain` should be used");
  }
  const array = [];
